...
async function createValidationDatabase(adminUrl) {
  const name = `zh_lao_v2_validation_${new Date().toISOString().replaceAll(/[-:.TZ]/g, '').slice(0, 14)}`;
...
export async function validate() {
  ...
  let databaseName;
  if (!connectionString) {
    const created = await createValidationDatabase(requireDatabaseUrl('ADMIN_DATABASE_URL'));
  ...
  const secondRun = await migrate(connectionString);
  if (secondRun.executed.length !== 0) throw new Error('Second migration run was not a no-op');
  const smokeTests = await runSmokeTests(connectionString);
  ...
  const report = await writeReport(connectionString);
  return {
    databaseName,
    firstRun,
    secondRun,
