...
export async function writeReport(connectionString = requireDatabaseUrl()) {
  ...
  const sections = [
    ...
    `- The original 121-table inventory plus approved forward migrations currently yields ${audit.actualBusinessTableCount} business tables. Content forward tables include revision history and the Content-owned idempotency replay ledger.`,
    '',
    '## Resolved blockers',
    '',
