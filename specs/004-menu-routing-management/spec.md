...
1. **Given** 运营人员已登录并拥有菜单管理权限,**When** 打开「菜单管理」页面,
   **Then** 系统以树形结构展示当前全部菜单项(分组、层级、顺序、可见性状态)。
2. **Given** 运营人员创建一个一级菜单项并指定其目标路由,
   **When** 保存,**Then** 系统持久化该菜单项,并可在菜单树中立即看到;侧边栏
   按配置渲染该菜单项。
3. **Given** 运营人员调整两个同级菜单项的先后顺序,**When** 保存,
   **Then** 系统按新顺序渲染侧边栏,顺序变更对所有有权限查看的运营人员生效。
...
- **FR-001**: 系统 MUST 允许拥有 `platform.menus.*` 权限的运营人员在后台以
  树形结构查看全部菜单项(分组、层级、顺序、可见性状态、目标)。
- **FR-002**: 系统 MUST 允许运营人员创建一级菜单项,指定显示名称、目标路由、
  图标标识、排序值与可见性权限要求列表(可为空,多权限 OR)。
- **FR-003**: 系统 MUST 允许运营人员在现有菜单项下创建子菜单项;菜单层级
  最大深度为 **3 层**(分组 → 一级项 → 子项),超出该深度的创建请求 MUST 被拒绝。
...
- **FR-006**: 系统 MUST 允许运营人员调整同级菜单项的排序,并持久化新顺序。
...
- **FR-011**: 系统 MUST 防止并发菜单编辑相互覆盖(乐观并发控制 / 版本校验)。
...
- **FR-016**: 系统 MUST 为菜单管理能力提供与现有 Platform 管理一致的
  后端管理接口(列表/创建/编辑/删除/排序/可见性),并复用现有认证与授权链路。

### Key Entities

- **菜单项 (Menu Item)**: 一条可导航的后台入口,含显示名称、层级位置(父级)、
  排序值、目标路由(白名单标识)、图标标识、**可见性权限要求列表(多权限 OR)**、
  启用状态、版本。组织为树形结构(分组 → 一级项 → 子项)。
...
- **图标**: 采用现有图标库(如 lucide-react 标识)作为可配置标识,不引入
  新图标系统。
- **层级深度**: 菜单最大深度为 3 层(分组 → 一级项 → 子项),与现有
  `SECONDARY_NAV` 的两级 + 分组结构一致并留一级扩展(用户已确认)。
