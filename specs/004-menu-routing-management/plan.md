...
### CR-001 增量（2026-09-04）

依据 ADR-024，菜单树位置不再决定节点类型。新增原子移动 use case 与 `POST .../:id/move`
端点：事务内验证节点/源层/目标层快照、拒绝环与超深子树、更新 `parent_id` 并压紧受影响层级。
管理页使用可访问的拖拽排序/换父级交互；导航规范化不再假设顶层节点没有路由。数据库表与
迁移保持不变。

### CR-002 增量（2026-09-04）

侧边栏含子项的顶层入口改为默认收起，路由激活态不再隐式改变展开状态；操作员通过入口按钮
显式切换。该变更只影响 Admin 呈现层和组件回归测试，不改变菜单、路由或权限契约。

### CR-003 增量（2026-09-04）

> 本节记录当时的兼容方案；其分组桥接策略已由下方 CR-004 和 ADR-026 取代。

`overview` 仍作为配置树中的兼容分组保留，但 Sidebar 不渲染其分组标题；「总览看板」继续
作为首个可直接导航的入口。该处理避免重写 seed 或运营人员已保存的菜单数据。

### CR-004 增量（2026-09-04）

采用 ADR-026 的统一递归目录树：删除前端分组适配层与三层业务限制，Sidebar、面包屑和权限
过滤递归处理任意深度；管理页允许任意节点继续新建子项或拖拽换父级。新增前向迁移 1330
归一化旧总览和内容桥接节点，保留现有表、API 权限、路由白名单与审计契约。

### CR-005 增量（2026-09-04）

可导航目录的链接行增加本地展开状态切换：点击菜单项目同时跳转和伸缩；箭头按钮继续只伸缩。
链接行仅在侧边栏展开且节点含子项时暴露 `aria-expanded`，整体折叠态不改变隐藏目录状态。
