...
description: 'Show the current Product Forge lifecycle status for a feature. Reads .forge-status.yml, displays all phases with completion state, lists available artifacts, and recommends the next action. Works for any feature in the features/ directory. Use with: "forge status", "show status", "/speckit.product-forge.status"'
