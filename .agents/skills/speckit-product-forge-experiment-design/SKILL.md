...
description: 'Optional Phase 9B: design a rigorous A/B experiment plan before shipping a feature behind a flag. Produces hypothesis, primary/secondary/guardrail metrics, minimum detectable effect, sample size calculation, exposure rules, and pre-registered decision thresholds. Wraps the installed `feature-flag-ab-testing` skill (or equivalent). Runs at Phase 9B — after Phase 9.5 (Monitoring Setup) and before deploy, when a flag-gated feature will be A/B tested. Use: "design experiment", "A/B plan", "/speckit.product-forge.experiment-design"'
