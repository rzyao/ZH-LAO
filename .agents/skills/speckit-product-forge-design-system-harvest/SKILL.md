...
description: 'Phase 2 helper: discover the project''s EXISTING design system (component library, design tokens, Storybook, Tailwind/CSS config) and emit a read-only manifest that grounds mockups, component decomposition, and verification in real code. The design system stays in code as the single source of truth; this command harvests a manifest, it never duplicates or rewrites the design system. Use with: "harvest design system", "/speckit.product-forge.design-system-harvest"'
