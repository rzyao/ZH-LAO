...
integration('Identity repositories on a fresh PostgreSQL V2 database', () => {
  ...
  it('serializes user, session, and OTP advisory locks without serializing another phone', async () => {
    ...
    let userSecond=false; let userSecondAt=0; let userFirstEnd=0; let userLocked!: () => void; const userReady=new Promise<void>(resolve=>{userLocked=resolve;}); const first=transactions.run(async executor=>{await createIdentityRepositories(executor).users.lockByInternalId(u.id);userLocked();await new Promise(resolve=>setTimeout(resolve,120));userFirstEnd=Date.now();}); await userReady; const second=transactions.run(async executor=>{await createIdentityRepositories(executor).users.lockByInternalId(u.id);userSecondAt=Date.now();userSecond=true;}); await Promise.all([first,second]); expect(userSecond).toBe(true); expect(userSecondAt).toBeGreaterThanOrEqual(userFirstEnd);
    let sessionSecond=false;let sessionSecondAt=0;let sessionFirstEnd=0;let sessionLocked!:()=>void;const sessionReady=new Promise<void>(resolve=>{sessionLocked=resolve;});const third=transactions.run(async executor=>{await createIdentityRepositories(executor).sessions.lockByRefreshTokenHash(parseRefreshTokenHash('lock-hash'));sessionLocked();await new Promise(resolve=>setTimeout(resolve,120));sessionFirstEnd=Date.now();});await sessionReady;const fourth=transactions.run(async executor=>{await createIdentityRepositories(executor).sessions.lockByRefreshTokenHash(parseRefreshTokenHash('lock-hash'));sessionSecondAt=Date.now();sessionSecond=true;});await Promise.all([third,fourth]);expect(sessionSecond).toBe(true);expect(sessionSecondAt).toBeGreaterThanOrEqual(sessionFirstEnd);
    let sameSecond=false;let sameSecondAt=0;let otherDone=false;let otpFirstEnd=0;let otpLocked!:()=>void;const otpReady=new Promise<void>(resolve=>{otpLocked=resolve;});const fifth=transactions.run(async executor=>{await createIdentityRepositories(executor).otpChallenges.acquireRequestAdvisoryLock(phone,'login');otpLocked();await new Promise(resolve=>setTimeout(resolve,120));otpFirstEnd=Date.now();});await otpReady;const sixth=transactions.run(async executor=>{await createIdentityRepositories(executor).otpChallenges.acquireRequestAdvisoryLock(phone,'login');sameSecondAt=Date.now();sameSecond=true;});const seventh=transactions.run(async executor=>{await createIdentityRepositories(executor).otpChallenges.acquireRequestAdvisoryLock(normalizePhoneNumber('+8562088888888'),'login');otherDone=true;});await Promise.all([fifth,sixth,seventh]);expect(sameSecond).toBe(true);expect(sameSecondAt).toBeGreaterThanOrEqual(otpFirstEnd);expect(otherDone).toBe(true);
  });
});
