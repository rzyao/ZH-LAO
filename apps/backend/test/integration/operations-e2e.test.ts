...
integration('Operations E2E with real Identity AuthenticationProvider',()=>{
 it('runs Identity JWT -> Operator -> role permission -> Operations mutation -> Audit and immediate revocation',async()=>{
  ...
  try{
   ...
   const me=await operationsApp.inject({method:'GET',url:'/api/v1/admin/operations/me',headers:bearer(rootIdentity.access_token)});expect(me.statusCode).toBe(200);const meData=success(me);expect(((meData.operator as Record<string,unknown>).permissions as Array<unknown>)).toHaveLength(68);
