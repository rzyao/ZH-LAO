...
integration('Operations E2E with real Identity AuthenticationProvider',()=>{
 it('runs Identity JWT -> Operator -> role permission -> Operations mutation -> Audit and immediate revocation',async()=>{
  ...
  try{
   ...
   const me=await operationsApp.inject({method:'GET',url:'/api/v1/admin/operations/me',headers:bearer(rootIdentity.access_token)});expect(me.statusCode).toBe(200);const meData=success(me);expect(((meData.operator as Record<string,unknown>).permissions as Array<unknown>)).toHaveLength(28);
   const created=await operationsApp.inject({method:'POST',url:'/api/v1/admin/operations/operators',headers:bearer(rootIdentity.access_token),payload:{auth_subject_id:workerIdentity.user_id,display_name:'Worker'}});expect(created.statusCode).toBe(200);expect(businessCode(created)).toBe('OK');const operatorId=String((success(created).operator as Record<string,unknown>).operator_id);
   ...
   const workerRead=await operationsApp.inject({method:'GET',url:'/api/v1/admin/operations/operators',headers:bearer(workerIdentity.access_token)});expect(workerRead.statusCode).toBe(200);expect(businessCode(workerRead)).toBe('OK');
   const workerDeniedCreate=await operationsApp.inject({method:'POST',url:'/api/v1/admin/operations/roles',headers:bearer(workerIdentity.access_token),payload:{code:'should_fail',name:'Denied'}});expect(workerDeniedCreate.statusCode).toBe(200);expect(businessCode(workerDeniedCreate)).toBe('FORBIDDEN');
   ...
   const workerDeniedDisabledRole=await operationsApp.inject({method:'GET',url:'/api/v1/admin/operations/operators',headers:bearer(workerIdentity.access_token)});expect(workerDeniedDisabledRole.statusCode).toBe(200);expect(businessCode(workerDeniedDisabledRole)).toBe('FORBIDDEN');
   ...
   const workerReadAgain=await operationsApp.inject({method:'GET',url:'/api/v1/admin/operations/operators',headers:bearer(workerIdentity.access_token)});expect(workerReadAgain.statusCode).toBe(200);expect(businessCode(workerReadAgain)).toBe('OK');
   ...
   const workerDeniedDisabledOperator=await operationsApp.inject({method:'GET',url:'/api/v1/admin/operations/operators',headers:bearer(workerIdentity.access_token)});expect(workerDeniedDisabledOperator.statusCode).toBe(200);expect(businessCode(workerDeniedDisabledOperator)).toBe('OPERATOR_DISABLED');
