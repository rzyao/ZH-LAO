...
export class PostgresContentAudioSourceReader implements ContentAudioSourceReader {
  ...
  private select(): string { return `SELECT c.content_type AS entity_type, c.public_id AS entity_id, cr.revision_public_id AS revision_id, c.language AS language_code, cr.status, CASE c.content_type WHEN 'lo_letter' THEN l.character WHEN 'lo_syllable' THEN ls.text WHEN 'lo_word' THEN lw.text WHEN 'lo_sentence' THEN lsent.text WHEN 'zh_pinyin' THEN zp.display_form WHEN 'zh_syllable' THEN zs.display_form END AS text_snapshot, cr.snapshot #> '{audio,pronunciation}' AS pronunciation_snapshot, COALESCE((cr.snapshot #>> '{audio,noAudio}')::boolean, false) AS no_audio FROM content.content_revisions cr JOIN content.contents c ON c.public_id = cr.entity_id LEFT JOIN content.lo_letters l ON l.content_id = c.id LEFT JOIN content.lo_syllables ls ON ls.content_id = c.id LEFT JOIN content.lo_words lw ON lw.content_id = c.id LEFT JOIN content.lo_sentences lsent ON lsent.content_id = c.id LEFT JOIN content.zh_pinyin zp ON zp.content_id = c.id LEFT JOIN content.zh_syllables zs ON zs.content_id = c.id`; }
}
