...
import { AccessTokenService, CryptoOtpGenerator, FakeFacebookCredentialVerifier, HmacOtpHasher, IdentityEventWriter, OtpConsumptionEngine, RefreshTokenService, type FacebookCredentialVerifier, type OtpDeliveryProvider } from '../application/services/index.js';
...
export function createIdentityHttpDependencies(options: IdentityHttpCompositionOptions): IdentityHttpDependencies {
  ...
  return {
    ...
    facebookAuth: new AuthenticateWithFacebook(options.facebookVerifier ?? new FakeFacebookCredentialVerifier(new Map()), options.transactionManager, options.repositories, { prepareRefresh: () => refresh.prepare(), issueAccess: user => access.issue(user) }, events, now),
