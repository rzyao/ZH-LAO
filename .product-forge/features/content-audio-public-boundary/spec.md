...
`sourceDomain` is `content`; slot identity is `(source_domain, content_entity_type, content_entity_id, language_code, audio_role)`. Eligible concrete types are `lo_letter`, `lo_syllable`, `lo_word`, `lo_sentence`, `zh_pinyin_element`, and `zh_syllable`. Course, lesson, exercise, question and generic `content` are rejected.
