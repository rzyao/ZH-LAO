...
- `POST /admin/rewards/grants/{grantNo}/void` 仅允许先检查 Grant/Delivery；若 Delivery 已 SUCCEEDED 返回业务错误 `REWARD_ALREADY_DELIVERED`。
...
```text
POST   /admin/rewards/programs
GET    /admin/rewards/programs
GET    /admin/rewards/programs/{id}
PATCH  /admin/rewards/programs/{id}
POST   /admin/rewards/programs/{id}/activate
POST   /admin/rewards/programs/{id}/pause
POST   /admin/rewards/programs/{id}/resume
POST   /admin/rewards/programs/{id}/end
POST   /admin/rewards/programs/{id}/archive
```
...
```text
POST   /admin/rewards/programs/{programId}/rules
GET    /admin/rewards/programs/{programId}/rules
GET    /admin/rewards/rules/{ruleId}
PATCH  /admin/rewards/rules/{ruleId}
POST   /admin/rewards/rules/{ruleId}/activate
POST   /admin/rewards/rules/{ruleId}/pause
POST   /admin/rewards/rules/{ruleId}/resume
POST   /admin/rewards/rules/{ruleId}/retire
POST   /admin/rewards/rules/{ruleId}/versions   -- 基于当前 Rule 创建下一版本
```
...
```text
GET /admin/rewards/events   ?source_domain=&event_type=&subject_user_id=&processing_status=&occurred_from=&occurred_to=&source_event_id=
GET /admin/rewards/events/{id}
POST /admin/rewards/events/{id}/retry     -- FAILED → RECEIVED，由正常 Worker 再处理
```
...
```text
GET /admin/rewards/grants ?user_id=&program_id=&rule_id=&reward_type=&decision_status=&delivery_status=&from=&to=
GET /admin/rewards/grants/{grantNo}           -- 对外用 grant_no，不用内部 bigint id
POST /admin/rewards/grants/{grantNo}/void

GET  /admin/rewards/deliveries ?status=&...
GET  /admin/rewards/deliveries/{id}
POST /admin/rewards/deliveries/{id}/retry     -- FAILED / 过期 PROCESSING → RETRY_WAIT，复用原 idempotency_key
POST /admin/rewards/deliveries/{id}/cancel    -- 仅 Grant=VOIDED 且 Delivery IN (PENDING, RETRY_WAIT)
```

- **禁止**：`POST /admin/rewards/grants` 直接给用户发 Coin、`POST /api/rewards/events`、`POST /api/me/rewards/{id}/claim`。
