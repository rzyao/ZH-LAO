...
领域文档的目录、侧边栏与中文显示规则遵循 [文档系统与领域文档规范](../governance/DOMAIN_DOCUMENT_STANDARD.md)。
...
| 领域 | 核心职责 | 已建立的产品功能 | 文档 |
| --- | --- | --- | --- |
| 身份（Identity） | 用户根、认证、Session、Device、基础资料与账号状态 | [登录与会话（主要）](/features/login/) | [身份领域](identity/) |
| 内容（Content） | Canonical 教学内容、课程、词汇、句子、练习定义与内容版本 | [音频生产（参与）](/features/audio-production/) | [内容领域](content/) |
| 学习（Learning） | 用户学习进度、掌握、复习、作答、活动与统计事实 | — | [学习领域](learning/) |
| 音频生产（Audio Production） | 业务音频生产、版本、审核、重试、批量生产与生产审计 | [音频生产（主要）](/features/audio-production/) | [音频生产领域](audio/) |
...
| 运营（Operations） | 后台 Operator、RBAC、角色权限与后台操作审计 | [音频生产（参与）](/features/audio-production/) | [运营领域](operations/) |
...
架构层的领域所有权见 [领域边界](../architecture/domains/)，跨领域调用和事件方向见 [领域依赖与协作](../architecture/domains/dependencies.md)。
