...
此会话将公开动态和互动纳入 Social 第一阶段。「全域审计修正版定稿」（消息 [52]，[D-135~D-138](../../governance/design-register.md)）已给出四张内容表的完整字段规格，字段级状态升级为 `frozen`；`social_reports` 已删除。
...
| ~~`social_reports`~~ | **已删除（`superseded`）**：举报事实统一归 `trust.reports`；Social 只保留举报入口 API | `superseded`（[D-115](../../governance/design-register.md)，Social 会话修正版定稿确认，[D-135](../../governance/design-register.md)） |
