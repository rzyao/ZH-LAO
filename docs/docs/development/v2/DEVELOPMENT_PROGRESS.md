...
Identity 已正式收口：`IDENTITY_IMPLEMENTATION = COMPLETE`、`IDENTITY_GATE = PASS`、`IDENTITY_DOMAIN = FROZEN`（IDN-01 ~ IDN-21 全部 `COMPLETE`）。后续 Domain Phase（Platform 等）与 Admin/Mobile 对 Identity 的依赖统一走 `identity/public` 与冻结 HTTP/API；任何下一 Phase 均不得自动开始，必须等待新的明确任务。
