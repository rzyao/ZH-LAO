...
<table class="ai-stage-table">
  <thead><tr><th>领域</th><th>功能</th><th v-for="lane in lanes" :key="lane">{{ laneLabels[lane] }}</th></tr></thead>
  <tbody><tr v-for="feature in features" :key="feature.id">
    <td class="domain-cell"><a v-if="domainHref(feature)" :href="domainHref(feature)">{{ domainLabel(feature) }}</a><span v-else>{{ domainLabel(feature) }}</span></td>
