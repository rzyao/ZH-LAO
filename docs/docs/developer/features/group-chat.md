...
本能力定义为 **群聊**（`group-chat`）。本页保留该能力的现有描述；如果现有资料没有明确用户价值，本页不自行补造。
...
该 Feature 已延期，不等于工程 `blocked`。当前 Chat canonical 只定义 Direct Conversation：同一用户对唯一 Direct Conversation、Direct 成员集合恒定，且 [Chat 域](/developer/reference/domains/chat/) 明确当前不包含群聊。[未决与延期事项](/developer/reference/governance/open-questions.md)进一步明确首期不建 `chat_group` / `chat_group_member`。
