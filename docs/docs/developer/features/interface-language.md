...
delivery_evidence:
- /developer/reference/evidence/mobile/MOBILE_FOUNDATION
---

# 界面语言切换

<!-- breadcrumb:start -->
> **← 返回** [unassigned](unassigned/) · [全量功能目录](index.md)
<!-- breadcrumb:end -->
...
本能力定义为 **界面语言切换**（`interface-language`）。本页保留该能力的现有描述；如果现有资料没有明确用户价值，本页不自行补造。
...
- [/developer/reference/evidence/mobile/MOBILE_FOUNDATION](/developer/reference/evidence/mobile/MOBILE_FOUNDATION)
...
`interface-language` 是当前正式 Feature 清单中的 Mobile Foundation 功能。Feature Inventory 记录已交付 实际 Stage / Gate 为设计、Mobile、验收；当前独立交付 surface 为 Mobile 与验收。
