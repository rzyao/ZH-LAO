...
本能力定义为 **错题本**（`wrong-answer-notebook`）。本页保留该能力的现有描述；如果现有资料没有明确用户价值，本页不自行补造。
...
参考：[Learning Use Cases](/developer/reference/contracts/learning/LEARNING_USE_CASES)、[Learning Design Audit](/developer/reference/evidence/learning/LEARNING_DESIGN_AUDIT)。
