...
本能力定义为 **运行参数管理**（`runtime-config-management`）。本页保留该能力的现有描述；如果现有资料没有明确用户价值，本页不自行补造。
...
本 Feature 负责真正跨领域的 Platform Runtime Config **当前值**治理。权威事实来自 [Platform 运行控制](/developer/reference/domains/platform/runtime-control.md)。Runtime Config 不是万能 `system_settings`：key 必须进入代码注册表并具有明确 owner / type / visibility / validation / fallback；历史、版本与回滚属于独立且延期的 [运行配置版本与回滚](/developer/features/runtime-config-history)。
