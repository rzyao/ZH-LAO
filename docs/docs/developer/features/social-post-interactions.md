...
本能力定义为 **动态点赞、评论与回复**（`social-post-interactions`）。本页保留该能力的现有描述；如果现有资料没有明确用户价值，本页不自行补造。
...
`social-post-interactions` 负责动态点赞 / 取消点赞、评论与回复的端到端交付跟踪。Like / Comment 的 canonical 事实来自 [Social 动态](/developer/reference/domains/social/community-content) 与 [Social 数据库](/developer/reference/domains/social/database)；举报仍只是 Social 的入口能力，举报事实唯一归 `trust.reports`。
