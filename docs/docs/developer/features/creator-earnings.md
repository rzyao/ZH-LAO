...
本能力定义为 **礼物收益、提现与结算**（`creator-earnings`）。本页保留该能力的现有描述；如果现有资料没有明确用户价值，本页不自行补造。
...
- [Commerce canonical](/developer/reference/domains/commerce/index.md)：领域职责明确不负责 Creator Economy / 提现 / 结算（V1），V1 范围同时明确延后 `Creator earnings` 与 `Withdrawal / Settlement`，且当前不建表。
