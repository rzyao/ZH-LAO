...
本能力定义为 **消息 Reaction**（`chat-reactions`）。本页保留该能力的现有描述；如果现有资料没有明确用户价值，本页不自行补造。
...
该 Feature 已延期，不等于工程 `blocked`。[Chat 域](/developer/reference/domains/chat/) 与 [未决与延期事项](/developer/reference/governance/open-questions.md)都明确当前模型不包含消息 Reaction，首期不建 `chat_message_reaction`。

这里记录的是当前真实边界，不是未来 Reaction 数据结构的预设计；未来重新激活时必须走新的 canonical 设计扩展。
