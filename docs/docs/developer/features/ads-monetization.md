...
本能力定义为 **广告变现**（`ads-monetization`）。本页保留该能力的现有描述；如果现有资料没有明确用户价值，本页不自行补造。
...
`ads-monetization` 是当前正式 Feature 清单中的功能。Feature Inventory 记录其参与领域为 [Learning](/developer/reference/domains/learning/index.md) 与 [Platform](/developer/reference/domains/platform/index.md)，当前交付 surface 为 Mobile、集成与验收；Portfolio 尚未启动该 Feature。
