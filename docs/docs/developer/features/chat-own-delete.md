...
本能力定义为 **单条消息仅自己删除**（`chat-own-delete`）。本页保留该能力的现有描述；如果现有资料没有明确用户价值，本页不自行补造。
...
[未决与延期事项](/developer/reference/governance/open-questions.md)明确首期不建用于单条私有删除的 `chat_message_user_state`。未来若激活必须作为新的明确设计扩展。
