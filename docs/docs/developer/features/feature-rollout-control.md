...
本能力定义为 **功能开关与范围灰度管理**（`feature-rollout-control`）。本页保留该能力的现有描述；如果现有资料没有明确用户价值，本页不自行补造。
...
本 Feature 负责 Platform 的 Feature Flag 定义、生命周期、运行时求值，以及 Region / Client / Region+Client 范围覆盖。权威产品事实来自 [Platform 运行控制](/developer/reference/domains/platform/runtime-control.md) 与 [Platform Domain](/developer/reference/domains/platform/)。V1 的范围灰度只覆盖冻结的范围模型；用户、分群、百分比、时间窗、版本表达式等高级策略属于 [高级灰度发布](/developer/features/advanced-feature-rollout)，不得回填到本 Feature。
