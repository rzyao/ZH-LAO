...
```text
0600_audio.sql
-> docs/developer/reference/domains/audio/database.md
-> ADR-020
...
The current trusted Brief itself names the current repository paths, including `database/migrations/0600_audio.sql`, the migrated contract/evidence snapshots, `apps/backend/src/modules/audio/public/` and the existing Admin/worker foundations. The stale path strings used by the contaminated finding are not requirements of the trusted Brief.
