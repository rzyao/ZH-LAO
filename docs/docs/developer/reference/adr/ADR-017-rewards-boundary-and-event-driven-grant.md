...
- 权益型奖励（会员天数、Follow 额度、曝光等 Entitlement）与 POINT/EXP/BADGE/GIFT/COUPON 资产、Manual Reward Grant、按用户时区奖励均延后（见[未决事项](/developer/reference/governance/open-questions.md)）。
- 旧「Contribution/Scoring（ContributionEvent、ScoreRecord）计分模型」被本决策取代，台账 D-017 标记 `superseded`。

## 事实源

- [Rewards 域](/developer/reference/domains/rewards/index.md)
- [Rewards 数据库 · 5 张表定稿](/developer/reference/domains/rewards/database.md)
- [Rewards 应用服务与事件](/developer/reference/domains/rewards/application-and-events.md)
- [PostgreSQL 总规范](/developer/reference/architecture/database.md)、[Domain Map](/developer/reference/architecture/domain-map.md)
...
- [设计台账](/developer/reference/governance/design-register.md)、[未决事项](/developer/reference/governance/open-questions.md)