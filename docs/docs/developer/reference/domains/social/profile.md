...
> 本页已按 Social 会话「全域审计修正版定稿」（消息 [45] 指令 + [52] 产出，[D-135~D-138](/developer/reference/governance/design-register.md)）更新：跨域引用统一 logical UUID、可被跨域引用的实体增加 `public_id`、软删除表改用 active-row partial UNIQUE。

## `social_profiles`

一行代表一个 User 的唯一公开社交身份；`user_id` 保存 Identity 的稳定 logical UUID，`UNIQUE(user_id)`，**无跨域物理 FK**（[D-136](/developer/reference/governance/design-register.md)，与 ADR-018 一致）。不复用 Basic Profile 昵称，也不把偏好、统计、关系或礼物塞入此表。
...
CHECK：母语时 `proficiency_level IS NULL`；非母语时 level 必填且在枚举中；不得同时 `is_native=true AND is_learning=true`。**每人最多一门母语**，由 partial UNIQUE 落实：`UNIQUE(profile_id) WHERE is_native = TRUE`（会话原设计即「每人只能一个母语」，修正版定稿将其明确为 partial UNIQUE，[D-137](/developer/reference/governance/design-register.md)）。资料发布时至少一门语言由服务层保证；Learning 可预填但创建后 Social 自主拥有数据。
