...
Social Schema 第一阶段冻结为 **19 张表**（原 20 张，`social_reports` 已删除，见下）；「全域审计修正版定稿」（消息 [45] 指令 + [52] 产出，[D-135~D-138](/developer/reference/governance/design-register.md)）为本版最终依据。字段和约束的唯一事实源在各规格页；本页只维护表清单和跨章节关系。
...
> **`social_reports` 已删除（`superseded`）。** Trust & Safety 会话的全域审计最终确认将 `trust.reports` 冻结为**全系统唯一** canonical user report fact；Social 只保留举报入口 API，不再持有举报事实表（[D-115](/developer/reference/governance/design-register.md)，与 [D-099](/developer/reference/governance/design-register.md) Canonical Fact 单一归属、[ADR-018](/developer/reference/adr/ADR-018-global-database-design-principles-final.md)「对已有定稿 Domain 的机械性修订指引」一致；Social 会话修正版定稿从 Social 侧正式确认，[D-135](/developer/reference/governance/design-register.md)）。表数量因此由 20 → 19。

## 跨域 ID 契约（修正版定稿）

- Social 内部继续使用 `BIGINT GENERATED ALWAYS AS IDENTITY` 主键与域内真实 FK；**BIGINT 不允许作为跨域契约暴露**（[D-136](/developer/reference/governance/design-register.md)，与 [D-097/D-098](/developer/reference/governance/design-register.md)、ADR-018 一致）。
...
- 外键、复合主键、CHECK 和 partial unique index 守护可由数据库表达的完整性；**跨 Domain 引用一律 logical UUID、零物理 FK**，表均位于 `social` Schema（[数据库总规范](/developer/reference/architecture/database.md)）。
- Follow 互关生成/结束 Match、审核后的资料发布资格（含 approved 主照片）、候选的双向偏好兼容、位置重排、内容可见性、Post「文字或图片至少一个」、评论 parent 归属和 Block 的联动拦截均由应用服务在事务中执行，不使用 Trigger，也不伪装成跨表 CHECK（[D-138](/developer/reference/governance/design-register.md)）。
...
`social_discovery_exposures` 为高增长行为事实：在线热数据保留最近 **90 天**，超期可归档或删除，第一阶段由定时清理任务实现；不做数据库 CHECK 或自动分区；长期统计需求进分析系统（[D-137](/developer/reference/governance/design-register.md)）。
