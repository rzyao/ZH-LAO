...
> 「拆分学习域」裁决（[ADR-021](/developer/reference/adr/ADR-021-content-and-learning-domain-split.md)，D-147/D-150）：Content 管「用户学什么」，Learning 管「用户学得怎么样」。本页只描述 **用户 × Content 的学习事实与状态**；知识结构、课程编排、题目定义、词典语义等 canonical 内容模型见 [Content 域](../content/index.md)。
