...
产品预期能力可按国家/地区、用户群、客户端版本和运营阶段分别开放（例如让两侧先看到 Feed，但只为某一侧开放 Match）。**V1 落地范围（已冻结）**：`feature_flag_overrides` 只支持 region / client_platform / region+client_platform 三种 scope；用户群灰度、版本表达式、百分比与时间阶段开放不在 V1 支持范围，属延期能力（见 [Platform 数据库](../domains/platform/database.md)）。
