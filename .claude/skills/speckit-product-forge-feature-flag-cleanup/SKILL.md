...
description: 'Cross-cutting stale-flag audit. Scans every `flags/registry.yml` produced by release-readiness across all features, cross-checks against the actual codebase, and produces a cleanup plan for flags whose `cleanup_after` date has passed. Wraps the installed `feature-flag-manager` skill. Safe to run on any schedule; read-only by default. Use: "clean up flags", "stale flags", "/speckit.product-forge.feature-flag-cleanup"'
