...
description: 'Brown-field entry point: reverse-engineer a Product Forge feature folder from existing code. Reads a target module/directory, synthesises retro product-spec, plan, and simplified tasks, then writes `.forge-status.yml` with all phases marked `completed` and `backfilled: true`. Produces a gaps-report.md listing what a modern Product Forge run would have required but is currently missing (tests, tracking, ADRs). Use: "backfill module", "create feature from existing code", "/speckit.product-forge.backfill"'
...
phases:
  ...
  # to a reverse-engineered feature. See .specify/extensions/product-forge/docs/policy.md §3 — skip-reason
