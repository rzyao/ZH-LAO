...
description: 'Phase 8B: Executes test cases via playwright-cli (interactive browser agent), tracks bugs in bugs/<BUG-NNN>.md, auto-fixes P0/P1 bugs and retests, performs gap analysis when bugs require spec changes. Loop continues until all P0/P1 bugs closed and exit criteria met. Use with: "run tests", "execute tests", "/speckit.product-forge.test-run"'
...
parser ([`.specify/extensions/product-forge/scripts/lib-yaml.js`](../scripts/lib-yaml.js)) reads it without a
dependency:

```bash
# Resolve {SELECTORS} for the journey — the CMP-* selectors the manifest owns.
# $PLUGIN_ROOT per .specify/extensions/product-forge/docs/runtime.md §1A; if lib-yaml.js is unreachable, fall back
